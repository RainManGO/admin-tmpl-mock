    ...
    stages {
        stage('Init'){
            steps {
                script{
                    // 【测试环境】变量
                    env.INGRESS_HOST='admin-tmpl-mock-test.rencaiyoujia.cn'
                    ...
                    if( "${BRANCH_NAME}"== "master"){
                        env.INGRESS_HOST='admin-tmpl-mock.rencaiyoujia.com'
                        env.INGRESS_TLS_SECRET="rencaiyoujia-tls-secret"
                        env.REPLICAS="2"
			...
			DOCKERFILE_NAME="Dockerfile"
                    ...
                    // 【开发环境】变量
                    if( "${BRANCH_NAME}"== "dev"){
                        env.INGRESS_HOST='admin-tmpl-mock-dev.rencaiyoujia.cn'
                        env.INGRESS_TLS_SECRET="rencaiyoujia-tls-secret"
                        env.REPLICAS="1"
        ...
        stage('DockerBuild') {
            steps {
                script{
                    devops.docker(env.NAME,dockerfile:"${DOCKERFILE_NAME}",context:".").build().push()
                }
            }
        }
